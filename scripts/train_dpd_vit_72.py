...
def build_dataset_samples(sanity_check=False):
    """
    Builds zero-leakage samples list for train, val, and test partitions.
    Supports Stratified Sub-sampling in Sanity Check mode.
    """
    class_names = load_class_names()
    class_to_idx = {c: i for i, c in enumerate(class_names)}
    local_pv_dir = r"Dataset/Plant_leave_diseases_dataset_without_augmentation"
    ...
    for c_idx, files in samples_by_class.items():
        ...
        if sanity_check:
            # Stratified Sanity Sub-sampling: 5 train, 2 val, 2 test per available class
            n_tr = min(5, n_files)
            ...
            # Full Dataset Stratified Split: 70% Train, 15% Val, 15% Test
...
# ---------------------------------------------------------
def train_pipeline(sanity_check=False, epochs_stage1=1, epochs_stage2=1):
    print("=" * 70)
    print(" 72-CLASS DPD VISION TRANSFORMER (ViT-Base/16) TRAINING & AUDIT PIPELINE ")
    print("=" * 70)
    ...
    train_samples, val_samples, test_samples, class_names = build_dataset_samples(sanity_check=sanity_check)
    ...
    # 4. Load Initialized ViT-Base Model
    print(f"\n--- Loading Initialized ViT-Base Model from '{INITIALIZED_CKPT_PATH}' ---")
    ...
    if os.path.exists(INITIALIZED_CKPT_PATH):
        ...
        print("Successfully loaded pre-initialized 72-class ViT checkpoint.")
    else:
        print("Warning: Initialized checkpoint not found. Starting from scratch initialization.")
    ...
    # Stage 2: Fine-Tuning Top Transformer Blocks
    print(f"\n==================================================")
    ...
    # ---------------------------------------------------------
    # Comprehensive Metric & Prediction Audit
    # ---------------------------------------------------------
    print(f"\n==================================================")
    print(f" FINAL HELD-OUT TEST METRIC & PREDICTION AUDIT ")
    print(f"==================================================")
    ...
    print("\nPredicted Class Counts Breakdown:")
    for up, pc in zip(unique_preds, pred_counts):
        print(f"  Class Index {up:>2} ({class_names[up]}): Predicted {pc} times")
    ...
    print(f"\nSaved audited test metrics to '{METRICS_SAVE_PATH}'")
    print("==================================================")
    print(" METRIC AUDIT & SANITY RUN VERIFICATION COMPLETED ")
    print("==================================================")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train 72-Class DPD ViT Model")
    parser.add_argument('--sanity-check', action='store_true', help="Run 1-epoch sanity check on stratified subset")
    parser.add_argument('--epochs-stage1', type=int, default=1, help="Stage 1 warmup epochs")
    parser.add_argument('--epochs-stage2', type=int, default=1, help="Stage 2 fine-tuning epochs")
